package uk.co.huydinh.app.champion.keyboard
{
	...
	public class OnScreenKeyboard extends Sprite
	{
		...
		{
			if (value is spark.components.TextInput) {
				var tlf:spark.components.TextInput = value as spark.components.TextInput;
		...
		{
			var char:String = "";
			var tlf:spark.components.TextInput;
			var text:String;
			var cursorPos:int;
			...
			if (focus) {
				if (focus is spark.components.TextInput) {
					tlf = focus as spark.components.TextInput;
					text = tlf.text;
					cursorPos = tlf.selectionAnchorPosition + 1;
					tlf.text = text.substring(0, tlf.selectionAnchorPosition) + char + text.substring(tlf.selectionActivePosition);
					tlf.selectRange(cursorPos, cursorPos);
					//tlf.textFlow.interactionManager.setFocus();
				}
			}
